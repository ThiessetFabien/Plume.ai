...
def generate_copilot_message(player_id: int, db: Session = Depends(get_db)):
    ...
    return {"player_id": player_id, "message": message_text}


@router.get(
    "/{player_id}/coaching-history", response_model=List[schemas.CoachingMessage]
)
def get_player_coaching_history(
    player_id: int, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)
):
